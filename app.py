...
if page == "Dashboard":
    ...
    with col_left:
        ...
        if total:
            scored = [t for t in hist if t["ml_score"] is not None]
            low = sum(1 for t in scored if t["ml_score"] < 0.4)
            med = sum(1 for t in scored if 0.4 <= t["ml_score"] < 0.7)
            high = sum(1 for t in scored if t["ml_score"] >= 0.7)
    ...
    with col_a:
        ...
        if alerts:
            for t in alerts:
                ...
                badge_text = "HIGH RISK" if (t["ml_score"] is not None and t["ml_score"] >= 0.7) else t["final_decision"].upper()
                st.markdown(
                    f"""
                    <div class="alert-row">
...
# =======================================================================
    ...
    if not alerts:
        ...
        for t in alerts:
            if t["ml_score"] is None:
                severity = "DATA REJECTED"
            elif t["ml_score"] >= 0.7:
                severity = "HIGH RISK"
            elif t["ml_score"] >= 0.4:
