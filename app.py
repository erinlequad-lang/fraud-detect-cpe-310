    ...
    with col_left:
        ...
        if total:
            ...
            fig = go.Figure(
                data=[
                    go.Pie(
                        labels=["Low Risk (0 to 0.4)", "Medium Risk (0.4 to 0.7)", "High Risk (0.7 to 1.0)"],
                        values=[low, med, high],
                        hole=0.68,
            ...
            for label, count, color in [
                ("Low Risk (0 to 0.4)", low, ui_theme.GREEN),
                ("Medium Risk (0.4 to 0.7)", med, ui_theme.AMBER),
                ("High Risk (0.7 to 1.0)", high, ui_theme.RED),
            ]:
                st.markdown(
                    f"""
...
# =======================================================================
    ...
    with form_col:
        ...
        st.caption(
            f"Rule Checking Agent will see this as **{'an odd hour (12am to 4:59am)' if hour <= 4 else 'a normal hour'}**, "
        ...
        st.markdown(
            ...
            <div class="panel" style="height:100%;">
                ...
                <div style="margin-top:6px; font-size:13px; color:{ui_theme.MUTED}; line-height:2.3;">
                    ...
                    <b style="color:{ui_theme.TEXT};">2. Pattern Spotting</b>: Random Forest model scores fraud risk (0 to 1)<br>
    ...
    if send_clicked:
        ...
        # Advance the counter so the NEXT transaction submitted gets a
        # different step value, and therefore a different transaction_id.
        # Without this, every transaction in the session reused the same
        # id (TXN00000001), so the Data Cleaning Agent saw every one
        # after the first as a duplicate and blocked it before it ever
        # reached the pattern, rule, or decision agents.
        st.session_state.step_counter += 1

        st.markdown("<div style='height:22px'></div>", unsafe_allow_html=True)
        st.markdown(
            f'<div class="panel-title" style="margin-bottom:2px;">Agent Pipeline Trace</div>'
            f'<div class="panel-sub" style="margin-bottom:16px;">Transaction {cleaned["transaction_id"]}, \u20A6{amount:,.2f} from {sender_name}</div>',
            unsafe_allow_html=True,
        )
        ...
        if not cleaned["is_clean"]:
            # Data that fails cleaning is rejected immediately. It is never
            # passed on to the pattern, rule, or decision agents, matching
            # the design in Chapter 3: a fault this early blocks the
            # transaction outright rather than being silently carried
            # forward with bad values.
            for i, (icon, label, filename) in enumerate([
                ...
                step_placeholders[i].markdown(
                    ui_theme.trace_step(
                        icon, "skip", label, filename,
                        "Skipped. The transaction was already rejected by the Data Cleaning Agent.",
                    ),
                    unsafe_allow_html=True,
                )
            ...
            result["rules_verdict"] = "not_checked"
            result["final_decision"] = "block"
            result["action_taken"] = "blocked_invalid_data"
            result["transaction_status"] = "blocked"
            result["alert_raised"] = True
            result["action_message"] = (
                "Transaction blocked before further processing because the Data Cleaning Agent "
                "rejected it as malformed or duplicate data."
            )
            result["action_timestamp"] = datetime.now().isoformat(timespec="seconds")
            ...
            st.markdown(
                ui_theme.final_verdict_banner(
                    ...
                    "The data cleaning step found a problem with this transaction, so it was blocked "
                    "before reaching the pattern, rule, or decision agents.",
                ),
                unsafe_allow_html=True,
            )
            ...
            rule_labels = {
                ...
                "odd_hour_transaction": "sent between 12:00 AM and 4:59 AM",
            }
            if triggered:
                triggered_html = "".join(
            ...
            if final_decision == "approve":
                ...
                # Store everything needed to resolve this later. Typing
                # into the verification box below triggers a fresh rerun
                # of the whole script, and a plain st.button/if-block only
                # stays true for the single rerun right after it is
                # clicked, so anything kept in a local variable here would
                # be lost the moment the person tries to enter the code.
                # Session state survives that rerun, which is what makes
                # the verification step actually work.
    ...
    # This block is intentionally outside "if send_clicked", so it keeps
    # showing the verification prompt (and correctly reacts to the code
    # being typed in) across reruns, instead of disappearing the instant
    # the Send button is no longer the thing that was just clicked.
    ...
    if pending:
        ...
        code = st.text_input("Verification code", max_chars=6, key="verification_code_input")

        if code:
            if code == "123456":
                sender["balance"] = new_balance
                result["final_decision"] = "approve"
                result["action_taken"] = "approved_after_verification"
                result["transaction_status"] = "approved_after_verification"
                result["action_message"] = "Transaction approved after the correct verification code was entered."
                st.success(f"Verified, \u20A6{amount:,.2f} sent. New balance: \u20A6{sender['balance']:,.2f}")
            else:
                result["final_decision"] = "block"
                result["action_taken"] = "blocked_failed_verification"
                result["transaction_status"] = "blocked"
                result["action_message"] = "Transaction blocked because the verification code entered was incorrect."
...
# =======================================================================
    ...
    agents = [
        ...
        ("2", "Pattern Spotting Agent", "Runs a trained Random Forest model on the transaction's PaySim features and produces ml_score, a 0 to 1 fraud likelihood.", "pattern_agent.py"),
