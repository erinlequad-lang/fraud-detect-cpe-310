...
def map_paysim_row(row, row_index):
    ...
    transaction_id combines the row's position with a high resolution
    timestamp, since relying on row_index alone collides across
    different browser sessions, every session's counter starts back at
    1, so two different people testing the live app (or the same
    person reloading the page) would otherwise eventually generate the
    exact same transaction_id and get wrongly rejected as a duplicate.
    timestamp is derived from "step" (1 step = 1 hour) against an
    arbitrary start date, since PaySim has no real dates.
    ...
    unique_suffix = f"{time.time_ns() % 100000000:08d}"

    transaction = {
        "transaction_id": f"TXN{row_index:04d}{unique_suffix}",
